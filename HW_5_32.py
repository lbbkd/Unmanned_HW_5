...
def main()->None:
    ...
    startime = Pathfinders.RRT(min_x,min_y,max_x,max_y,gs,domain_x,domain_y,domain_gs)
    wp_list = startime.main(Goal_x,Goal_y,Start_x,Start_y,Obstacle_x,Obstacle_y,)
    wp_list = dict(reversed(list(wp_list.items())))
    for i in wp_list:
        print(wp_list[i].x,wp_list[i].y,wp_list[i].parent_index,i)
   ...
   #set a desired heading angle
    ...
    error_tolerance = numpy.deg2rad(0.1)
